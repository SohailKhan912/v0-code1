...
interface PivotFrameSystemProps {
  width: number
  height: number
  material: string
  finish: string
...
export function PivotFrameSystemModel({ width, height, material, finish, features }: PivotFrameSystemProps) {
  ...
  useEffect(() => {
    ...
    import("three").then(
      ({
        ...
        const pivotAluminumMaterial = new MeshStandardMaterial({
          color: 0xb8b8b8,
          metalness: 0.8,
          roughness: 0.15,
        })
        ...
        const topFrame = new Mesh(topFrameGeometry, pivotAluminumMaterial)
        topFrame.position.y = doorHeight / 2 + frameThickness / 2
        topFrame.castShadow = true
        doorGroup.add(topFrame)

        // Bottom pivot frame
        const bottomFrame = new Mesh(topFrameGeometry, pivotAluminumMaterial)
        bottomFrame.position.y = -doorHeight / 2 - frameThickness / 2
        bottomFrame.castShadow = true
        doorGroup.add(bottomFrame)

        // Pivot hinges (top and bottom center)
        ...
        const pivotGeometry = new CylinderGeometry(0.08, 0.08, 0.15, 32)
        const topPivot = new Mesh(pivotGeometry, pivotMaterial)
        topPivot.position.y = doorHeight / 2 + 0.15
        topPivot.castShadow = true
        doorGroup.add(topPivot)

        const bottomPivot = new Mesh(pivotGeometry, pivotMaterial)
        bottomPivot.position.y = -doorHeight / 2 - 0.15
        bottomPivot.castShadow = true
        doorGroup.add(bottomPivot)
        ...
        const handleGeometry = new BoxGeometry(0.09, 0.22, 0.04)
        const handle = new Mesh(handleGeometry, handleMaterial)
        handle.position.set(doorWidth / 2 + 0.1, 0, 0.09)
